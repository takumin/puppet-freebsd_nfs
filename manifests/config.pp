...
class freebsd_nfs::config {
  if $::freebsd_nfs::server_enable or $::freebsd_nfs::client_enable {
    ...
    if $::freebsd_nfs::use_nfsv4 {
      ...
      concat::fragment { 'export_nfsv4':
        ...
        content => "V4:${::freebsd_nfs::export_nfsv4} $args",
        order   => '01'
      }
    }

    if defined($::freebsd_nfs::mountd_flags) {
      sysrc { 'rpcbind_flags':
        value => $::freebsd_nfs::rpcbind_flags,
      }
    }

    if defined($::freebsd_nfs::rpc_lockd_flags) {
      sysrc { 'rpc_lockd_flags':
        value => $::freebsd_nfs::rpc_lockd_flags,
      }
    }

    if defined($::freebsd_nfs::rpc_statd_flags) {
    ...
    if $::freebsd_nfs::use_nfsv4 {
      if !validate_re($::freebsd_nfs::nfsuserd_flags, '-domain') {
        sysrc { 'nfsuserd_flags':
          value => "-domain ${::domain}",
        }
      }
    } else {
      if defined($::freebsd_nfs::nfsuserd_flags) {
        sysrc { 'nfsuserd_flags':
          value => $::freebsd_nfs::nfsuserd_flags,
        }
      }
    }
  }
  ...
  if $::freebsd_nfs::server_enable {
    ...
    if defined($::freebsd_nfs::weak_mountd_authentication) {
      sysrc { 'weak_mountd_authentication':
        value => $::freebsd_nfs::weak_mountd_authentication,
      }
    }

    if defined($::freebsd_nfs::nfs_server_flags) {
      sysrc { 'nfs_server_flags':
        value => $::freebsd_nfs::nfs_server_flags,
      }
    }

    if defined($::freebsd_nfs::nfs_reserved_port_only) {
      sysrc { 'nfs_reserved_port_only':
        value => $::freebsd_nfs::nfs_reserved_port_only,
      }
    }

    if defined($::freebsd_nfs::oldnfs_server_enable) {
      sysrc { 'oldnfs_server_enable':
        value => $::freebsd_nfs::oldnfs_server_enable,
      }
    }
    ...
    if $::freebsd_nfs::use_nfsv4 {
      ...
      if defined($::freebsd_nfs::nfsv4_server_enable) {
        sysrc { 'nfsv4_server_enable':
          value => $::freebsd_nfs::nfsv4_server_enable,
        }
      }
    }
  }
